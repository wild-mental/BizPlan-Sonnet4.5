...
> **프로모션 기간:** 2024년 12월 29일 ~ 2025년 1월 11일
> **혜택:** 2단계 사전등록 할인
> **목표:** 잠재 수요 검증 및 사전등록 회원 확보
...
| 기간 | 할인율 | 설명 |
|------|--------|------|
| **Phase A: 연말연시 특별** | 🔥 **30% 할인** | 2024-12-29 ~ 2025-01-04 (7일간) |
| **Phase B: 얼리버드 특가** | ✨ **10% 할인** | 2025-01-05 ~ 2025-01-11 (7일간) |

> 📢 **프로모션 일정:** Phase A (연말연시 특별) → Phase B (얼리버드 특가) 순차 진행

---

## 📊 할인 요금 계산

### 🔥 Phase A: 연말연시 특별 30% 할인 (12/29 ~ 1/4)

| 요금제 | 정가 | 30% 할인가 | 절약 금액 |
|--------|------|-----------|----------|
| 플러스 | ₩399,000 | **₩279,300** | ₩119,700 |
| 프로 | ₩799,000 | **₩559,300** | ₩239,700 |
| 프리미엄 | ₩1,199,000 | **₩839,300** | ₩359,700 |

### ✨ Phase B: 얼리버드 특가 10% 할인 (1/5 ~ 1/11)

| 요금제 | 정가 | 10% 할인가 | 절약 금액 |
|--------|------|-----------|----------|
| 플러스 | ₩399,000 | **₩359,100** | ₩39,900 |
| 프로 | ₩799,000 | **₩719,100** | ₩79,900 |
| 프리미엄 | ₩1,199,000 | **₩1,079,100** | ₩119,900 |

### 📈 할인율 비교 요약

| 요금제 | 정가 | 30% 할인 (연말연시) | 10% 할인 (얼리버드) | 추가 절약 (연말연시) |
|--------|------|-------------------|-------------------|---------------------|
| 플러스 | ₩399,000 | ₩279,300 | ₩359,100 | +₩79,800 |
| 프로 | ₩799,000 | ₩559,300 | ₩719,100 | +₩159,800 |
| 프리미엄 | ₩1,199,000 | ₩839,300 | ₩1,079,100 | +₩239,800 |
...
2. **내용:**
   - 왼쪽: 할인율 배지 (연말연시 30% / 얼리버드 10% 자동 전환)
   - 중앙: "🔥 연말연시 특별 30% 할인" 또는 "✨ 얼리버드 특가 10% 할인" + 카운트다운
   - 오른쪽: "지금 등록하기" CTA 버튼 + 닫기(X) 버튼
3. **스타일:**
   - 연말연시(30%): 배경 그라데이션 (rose-500 → orange-500) - 더 강렬한 색상
   - 얼리버드(10%): 배경 그라데이션 (emerald-500 → cyan-500)
   - 텍스트: 흰색, font-medium
   - 높이: 48px (모바일), 40px (데스크톱)
4. **기능:**
   - 닫기 버튼 클릭 시 localStorage에 저장하여 24시간 숨김
   - 2단계 카운트다운:
     - 1/4 23:59:59까지: "30% 할인 마감까지"
...
```tsx
// 연말연시 30% 할인 마감 (1/4까지)
const phase1 = useCountdown('2025-01-04T23:59:59+09:00');

// 얼리버드 10% 할인 마감 (1/11까지)
...
1. **헤더:**
   ...
   - 서브타이틀: 
     - 연말연시(~1/4): "🔥 지금 등록하면 30% 할인 혜택!"
     - 얼리버드(1/5~): "✨ 지금 등록하면 10% 할인 혜택!"
...
2. **요금제 선택 카드:**
   ...
   - 정가 취소선 + 현재 할인가 강조
   - 연말연시 등록 시 추가 절약 금액 표시 (vs 얼리버드 등록)
...
const pricingPlans = [
  ...
  { name: '프리미엄', price: '1,199,000', period: '2026 상반기 시즌', features: [...], cta: '프리미엄 시작', popular: false },
];
```
...
1. **할인 배지:**
   ...
   - 연말연시(~1/4): "🔥 30% OFF" (rose-500 배경)
   - 얼리버드(1/5~): "✨ 10% OFF" (emerald-500 배경)
   - 애니메이션: 살짝 흔들리는 효과 (wiggle)

2. **가격 표시:**
   - 정가: 취소선 + 회색 텍스트
   - 할인가: 크게 표시 (연말연시: rose, 얼리버드: emerald)
   - 절약 금액: 
     - 연말연시: "₩119,700 절약!" (플러스 기준)
     - 얼리버드: "₩39,900 절약!"
   - 연말연시 한정: "얼리버드보다 ₩79,800 더 절약!" 추가 표시
...
- 프로모션 종료일은 상수로 관리:
  - PHASE_A_END: '2025-01-04T23:59:59+09:00' (30% 마감)
...
```json
{
  ...
  "currentPhase": "A",
  "discountRate": 30,
  "phaseAEnd": "2025-01-04T23:59:59+09:00",
  "phaseBEnd": "2025-01-11T23:59:59+09:00",  // 얼리버드 특가 마감
  "prices": {
    "plus": { "original": 399000, "discounted": 279300 },
    "pro": { "original": 799000, "discounted": 559300 },
    "premium": { "original": 1199000, "discounted": 839300 }
  }
}
```
...
- **연말연시 특별 (12/29~1/4):** 30% 할인 - 플러스 ₩279,300 / 프로 ₩559,300 / 프리미엄 ₩839,300
- **얼리버드 특가 (1/5~1/11):** 10% 할인 - 플러스 ₩359,100 / 프로 ₩719,100 / 프리미엄 ₩1,079,100

### Q2. 사전 등록 기간은 언제까지인가요?
A: 2025년 12월 28일부터 2026년 정부지원사업 접수 시작일까지입니다.
- **연말연시 30% 할인:** 1월 4일 23:59까지
- **얼리버드 10% 할인:** 1월 11일 23:59까지
...
### 2. 프로모션 종료 D-3 리마인드
- 제목: "[Makers Round] 얼리버드 특가 마감 3일 전!"
- 내용:
  - 마감 임박 안내
  - 아직 등록 안 했다면 CTA
  - 이미 등록했다면 친구 추천 안내
...
*Created: 2025-12-26*
*Last Updated: 2024-12-28 (할인 기간 및 할인율 조정: 30% 12/29~1/4, 10% 1/5~1/11)*
