...
> **프로모션 기간:** 2025년 12월 26일 ~ 2026년 1월 첫째 주 (1월 5일)
> **혜택:** 전 요금제 20% 할인 사전 등록
...
2. **내용:**
   - 왼쪽: "🎉 얼리버드 20% 할인 - 2026 상반기 시즌 사전 등록"
   - 중앙: 카운트다운 타이머 (일:시:분:초)
   - 오른쪽: "지금 등록하기" CTA 버튼 + 닫기(X) 버튼
3. **스타일:**
   - 배경: 그라데이션 (emerald-500 → cyan-500)
   - 텍스트: 흰색, font-medium
   - 높이: 48px (모바일), 40px (데스크톱)
4. **기능:**
   - 닫기 버튼 클릭 시 localStorage에 저장하여 24시간 숨김
   - 프로모션 종료일(2026-01-05 23:59:59 KST) 기준 카운트다운
...
## Example Usage
```tsx
const { days, hours, minutes, seconds, isExpired } = useCountdown('2026-01-05T23:59:59+09:00');
```
```
...
## Task
요금제 카드에 20% 할인 프로모션 정보를 추가해주세요.

## Requirements
1. **할인 배지:**
   - 카드 우상단에 "20% OFF" 배지 (회전된 리본 스타일)
   - 애니메이션: 살짝 흔들리는 효과 (wiggle)

2. **가격 표시:**
   - 정가: 취소선 + 회색 텍스트
   - 할인가: 크게 표시 + emerald 색상
   - 절약 금액: "₩79,800 절약" 작은 텍스트
...
## Technical Constraints
- pricingPlans 데이터에 originalPrice, discountedPrice 필드 추가
...
A: 정식 오픈 전 사전 등록하시면 전 요금제 20% 할인 혜택을 받으실 수 있습니다. 
플러스 요금제는 ₩319,200, 프로 요금제는 ₩639,200, 프리미엄 요금제는 ₩1,199,200에 이용 가능합니다.

### Q2. 사전 등록 기간은 언제까지인가요?
A: 2025년 12월 26일부터 2026년 1월 5일까지입니다. 
기간 내 등록하신 분께만 20% 할인 코드가 발급됩니다.
